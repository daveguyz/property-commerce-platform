var PCP=function(){"use strict";const y={apiUrl:"",wsUrl:"",tenantId:null,apiKey:null,currency:"USD",locale:"en",theme:{primaryColor:"#6366f1",primaryTextColor:"#ffffff",fontFamily:"inherit",borderRadius:"8px"},mockMode:!1};let p={...y},k=!1;function b(e={}){const t=typeof window<"u"&&window.PCPConfig||{};return p={...y,...t,...e,theme:{...y.theme,...t.theme||{},...e.theme||{}}},p.apiUrl||(p.mockMode=!0),T(p.theme),k=!0,p}function u(){return k||b(),p}function T(e){if(typeof document>"u")return;const t=document.documentElement;t.style.setProperty("--pcp-primary-color",e.primaryColor),t.style.setProperty("--pcp-primary-text-color",e.primaryTextColor),t.style.setProperty("--pcp-font-family",e.fontFamily),t.style.setProperty("--pcp-border-radius",e.borderRadius)}function F(){return u().mockMode===!0}async function B(){var t,n,a;const e=u();if(!e.tenantId||e.mockMode)return e;try{const r=await fetch(`${e.apiUrl.replace(/\/$/,"")}/api/v1/tenants/${e.tenantId}/config`);if(!r.ok)return e;const o=await r.json(),i=(o==null?void 0:o.data)||{},c=typeof window<"u"&&((t=window.PCPConfig)==null?void 0:t.theme)||{};p={...p,currency:((n=window.PCPConfig)==null?void 0:n.currency)||i.currency||p.currency,locale:((a=window.PCPConfig)==null?void 0:a.locale)||i.locale||p.locale,features:i.features||{},theme:{...p.theme,...i.theme||{},...c}},T(p.theme)}catch{}return p}const h="pcp_token",_="pcp_refresh",d={getToken(){return P(h)},setToken(e){D(h,e)},getRefresh(){return P(_)},setRefresh(e){D(_,e)},clear(){A(h),A(_)},isAuthenticated(){return!!this.getToken()},getRoles(){const e=this.getToken();if(!e)return[];try{const t=JSON.parse(atob(e.split(".")[1])),n=t.roles||t.authorities||[];return Array.isArray(n)?n.map(a=>String(a).toLowerCase()):[]}catch{return[]}},getUserId(){const e=this.getToken();if(!e)return null;try{const t=JSON.parse(atob(e.split(".")[1]));return t.sub||t.userId||null}catch{return null}},headers(){const e=this.getToken();return e?{Authorization:`Bearer ${e}`}:{}}};function P(e){try{return localStorage.getItem(e)}catch{return null}}function D(e,t){try{localStorage.setItem(e,t)}catch{}}function A(e){try{localStorage.removeItem(e)}catch{}}function j(e,t={}){return e.includes("/auctions")&&!e.includes("/questions")?Promise.resolve(q(e)):e.includes("/properties/search")||e.includes("/properties?")?Promise.resolve(Y()):Promise.resolve({success:!0,data:null})}function q(e){const t=["Oakwood Residences","The Commercial Quarter","Riverside Business Park","Harbour View Apartments","The Merchant Centre","Estate at Millfield"],n=["London","New York","Sydney","Cape Town","Dubai","Singapore"],a=["OPEN","SCHEDULED","OPEN","EXTENDED","SCHEDULED","CLOSED"],r=t.map((c,s)=>({id:`mock-lot-${s+1}`,title:c,propertyId:`mock-prop-${s+1}`,status:a[s],auctionType:["ENGLISH","DUTCH","ENGLISH","SEALED_BID","ENGLISH","ENGLISH"][s],startingPrice:[45e4,82e4,31e4,59e4,12e5,275e3][s],currentBidAmount:a[s]==="OPEN"||a[s]==="EXTENDED"?[475e3,0,34e4,615e3,0,0][s]:null,winningAmount:a[s]==="CLOSED"?29e4:null,currency:"USD",uniqueBidders:[4,0,2,7,0,0][s],totalBids:[11,0,5,19,0,3][s],startsAt:new Date(Date.now()+(s-2)*864e5).toISOString(),scheduledEndsAt:new Date(Date.now()+(s%2===0?1:-1)*36e5+65e3).toISOString(),location:{city:n[s]}}));let o=r;const i=e.match(/status=([^&]+)/);if(i){const c=decodeURIComponent(i[1]).split(",");o=r.filter(s=>c.includes(s.status))}return{success:!0,data:{content:o,totalElements:o.length,totalPages:1,page:0,size:12}}}function Y(){return{success:!0,data:{content:[1,2,3,4,5,6].map(e=>({id:`mock-${e}`,title:["Oakwood Residences","The Commercial Quarter","Riverside Business Park","Harbour View Apartments","The Merchant Centre","Estate at Millfield"][e-1],location:{city:["London","New York","Sydney","Cape Town","Dubai","Singapore"][e-1]},pricing:{currentDynamicRate:[85e4,12e5,475e3,62e4,98e4,32e4][e-1],currency:"USD"},bedrooms:[2,3,4,1,3,2][e-1],maxGuests:[4,6,8,2,6,4][e-1],averageRating:[4.9,4.7,4.8,4.6,4.5,4.4][e-1],totalReviews:[42,28,61,15,19,8][e-1],imageUrls:[],status:"ACTIVE"})),totalElements:6,page:0,size:6}}}let m=null;async function w(e,t={}){const n=u();if(F())return j(e,t);const a=n.apiUrl.replace(/\/$/,"")+e,r=await fetch(a,{...t,headers:G(n,t)});if(r.status===401){if(await V(n))return w(e,t);d.clear(),K()}const o=await r.json().catch(()=>({}));if(!r.ok){const i=new Error(o.message||`API error ${r.status}`);throw i.status=r.status,i.data=o,i}return o}function G(e,t){const n={"Content-Type":"application/json",...e.tenantId?{"X-Tenant-Id":e.tenantId}:{},...d.headers(),...t.headers||{}};return!d.getToken()&&e.apiKey&&(n["X-Api-Key"]=e.apiKey),n}async function V(e){const t=d.getRefresh();return t?(m||(m=fetch(e.apiUrl.replace(/\/$/,"")+"/api/v1/auth/refresh",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({refreshToken:t})}).then(n=>n.json()).then(n=>{var a;return n.success&&((a=n.data)!=null&&a.accessToken)?(d.setToken(n.data.accessToken),d.setRefresh(n.data.refreshToken),!0):!1}).catch(()=>!1).finally(()=>{m=null})),m):!1}function K(){typeof document>"u"||document.dispatchEvent(new CustomEvent("pcp:auth-expired"))}let l=null;function W(){return l&&document.body.contains(l)||(l=document.createElement("div"),l.id="pcp-toast-container",l.setAttribute("aria-live","polite"),l.style.cssText=["position:fixed","bottom:20px","right:20px","z-index:99999","display:flex","flex-direction:column","gap:8px","font-family:var(--pcp-font-family, inherit)"].join(";"),document.body.appendChild(l)),l}function L(e,t="default",n=4e3){if(typeof document>"u")return;const a=W(),r=document.createElement("div");r.setAttribute("role","status"),r.textContent=e;const o={default:{bg:"#1f2937",fg:"#ffffff"},success:{bg:"#15803d",fg:"#ffffff"},error:{bg:"#b91c1c",fg:"#ffffff"},warning:{bg:"#92400e",fg:"#ffffff"}}[t]||{bg:"#1f2937",fg:"#ffffff"};r.style.cssText=[`background:${o.bg}`,`color:${o.fg}`,"padding:10px 16px","border-radius:var(--pcp-border-radius, 8px)","font-size:14px","box-shadow:0 4px 16px rgba(0,0,0,.2)","max-width:320px","animation:pcp-toast-in .2s ease"].join(";"),a.appendChild(r),setTimeout(()=>r.remove(),n)}function X(e){return String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const g=new Map,x=new Map;function $(e,t){if(typeof(t==null?void 0:t.mount)!="function")throw new Error(`PCP widget "${e}" must export a mount(el, props, ctx) function`);g.set(e,t)}function J(e,t,n={},a){const r=g.get(e);if(!r)return console.error(`[PCP] Unknown widget "${e}". Registered widgets: ${[...g.keys()].join(", ")||"(none loaded)"}`),null;const o=typeof t=="string"?document.querySelector(t):t;if(!o)return console.error(`[PCP] mount target not found: ${t}`),null;E(o);const i=r.mount(o,n,a),c=o;return x.set(c,{name:e,cleanup:i}),o.dataset.pcpWidget=e,{unmount:()=>E(o)}}function Z(e){const t=typeof e=="string"?document.querySelector(e):e;t&&E(t)}function E(e){const t=x.get(e);if(t){if(typeof t.cleanup=="function")try{t.cleanup()}catch(n){console.warn(`[PCP] Cleanup error for widget "${t.name}":`,n)}x.delete(e),delete e.dataset.pcpWidget,e.innerHTML=""}}function Q(){return[...g.keys()]}const I={USD:{symbol:"$",name:"US Dollar"},EUR:{symbol:"€",name:"Euro"},GBP:{symbol:"£",name:"British Pound"},JPY:{symbol:"¥",name:"Japanese Yen"},AUD:{symbol:"A$",name:"Australian Dollar"},CAD:{symbol:"C$",name:"Canadian Dollar"},CHF:{symbol:"Fr",name:"Swiss Franc"},CNY:{symbol:"¥",name:"Chinese Yuan"},INR:{symbol:"₹",name:"Indian Rupee"},ZAR:{symbol:"R",name:"South African Rand"},AED:{symbol:"د.إ",name:"UAE Dirham"}},N={en:"en-US",fr:"fr-FR",es:"es-ES",de:"de-DE",pt:"pt-BR",ar:"ar-SA",zh:"zh-CN"};function R(e){var t;return((t=I[e])==null?void 0:t.symbol)||e}function ee(e){const{currency:t,locale:n}=u(),a=N[n]||"en-US";try{return new Intl.NumberFormat(a,{style:"currency",currency:t,maximumFractionDigits:0}).format(Number(e)||0)}catch{return R(t)+Number(e||0).toLocaleString()}}function te(e,t={}){if(!e)return"";const{locale:n}=u(),a=N[n]||"en-US",r=t.timezone||Intl.DateTimeFormat().resolvedOptions().timeZone;try{return new Intl.DateTimeFormat(a,{timeZone:r,day:"numeric",month:"short",hour:t.withTime?"2-digit":void 0,minute:t.withTime?"2-digit":void 0,year:t.withYear!==!1?"numeric":void 0}).format(new Date(e))}catch{return new Date(e).toLocaleString()}}const U=Object.freeze(Object.defineProperty({__proto__:null,CURRENCIES:I,currencySymbol:R,fmt:ee,fmtDate:te},Symbol.toStringTag,{value:"Module"}));b();function ne(){return{api:w,auth:d,i18n:U,toast:L,escapeHtml:X,config:u()}}const z={init(e){return b(e),B()},config:u,mount(e,t,n){return J(e,t,n,ne())},unmount:Z,registerWidget:$,listWidgets:Q,api:w,auth:d,i18n:U,toast:L,version:"2.0.0"};typeof window<"u"&&(window.PCP=z,document.dispatchEvent(new CustomEvent("pcp:ready")));const v=new Set;function ae(e,t){if(v.has(e)||typeof document<"u"&&document.getElementById(e)){v.add(e);return}if(typeof document>"u")return;const n=document.createElement("style");n.id=e,n.textContent=t,document.head.appendChild(n),v.add(e)}const re=`
.pcp-auction-listing {
...
`,oe={ENGLISH:"📈",DUTCH:"📉",REVERSE:"🔄",SEALED_BID:"🔒"},ie={ENGLISH:"English",DUTCH:"Dutch",REVERSE:"Reverse",SEALED_BID:"Sealed"},se={mount(e,t,n){ae("pcp-auction-listing-styles",re);const a={statuses:t.statuses||["OPEN","EXTENDED"],type:t.type||null,page:0,totalPages:0,timers:[],roomUrlTemplate:t.roomUrl||"#lot={lotId}"};return e.className=(e.className?e.className+" ":"")+"pcp-auction-listing",e.innerHTML=ce(),pe(e,a,n),S(e,a,n),()=>{a.timers.forEach(r=>clearInterval(r))}}};function ce(){return`
  ...
  `}function pe(e,t,n){e.querySelectorAll(".pcp-al__tab").forEach(a=>{a.addEventListener("click",()=>{e.querySelectorAll(".pcp-al__tab").forEach(r=>{r.classList.remove("pcp-al__tab--active"),r.setAttribute("aria-selected","false")}),a.classList.add("pcp-al__tab--active"),a.setAttribute("aria-selected","true"),t.statuses=a.dataset.statuses.split(","),t.page=0,S(e,t,n)})})}async function S(e,t,n){var r,o;const a=e.querySelector(".pcp-al__grid");a.innerHTML=le(),t.timers.forEach(i=>clearInterval(i)),t.timers=[];try{const i=new URLSearchParams({status:t.statuses.join(","),page:t.page,size:12,...t.type?{type:t.type}:{}}),c=await n.api(`/api/v1/auctions?${i}`),s=((r=c==null?void 0:c.data)==null?void 0:r.content)||[];if(t.totalPages=((o=c==null?void 0:c.data)==null?void 0:o.totalPages)||1,!s.length){a.innerHTML='<p class="pcp-al__empty">No auctions found for this filter.</p>',H(e,t,n);return}a.innerHTML=s.map(f=>de(f,n,t)).join(""),ue(a,t),H(e,t,n)}catch{a.innerHTML='<p class="pcp-al__error">Could not load auctions. Please try again.</p>'}}function le(){return Array.from({length:6}).map(()=>`
      ...
      </div>`).join("")}function de(e,t,n){var M;const{escapeHtml:a,i18n:r}=t,o=e.status==="OPEN"||e.status==="EXTENDED",i=["CLOSED","SETTLED","NO_RESERVE","CANCELLED"].includes(e.status),c=o&&e.currentBidAmount?e.currentBidAmount:e.startingPrice,s=i&&e.winningAmount?e.winningAmount:c,f=o?"Current bid":i?"Winning bid":"Starting",C=n.roomUrlTemplate.replace("{lotId}",e.id),fe=o?'<span class="pcp-al__live-badge"><span class="pcp-al__live-dot"></span>Live</span>':"",me=o&&e.scheduledEndsAt?`<div class="pcp-al__countdown" data-ends-at="${e.scheduledEndsAt}" role="timer" aria-label="Time remaining">
           ${["d","h","m","s"].map((be,O)=>`
             <span class="pcp-al__cd-seg"><b data-part="${be}">--</b><small>${["d","h","m","s"][O]}</small></span>${O<3?"<i>:</i>":""}`).join("")}
         </div>`:e.startsAt?`<p class="pcp-al__timing">Starts ${r.fmtDate(e.startsAt,{withTime:!0})}</p>`:"",ge=o?"Bid now":i?"View results":"View lot",ye=o?"pcp-btn--primary":"pcp-btn--ghost";return`
    <article class="pcp-al__card${i?" pcp-al__card--closed":""}" data-lot-id="${a(e.id)}">
      <a href="${C}" class="pcp-al__img-link" tabindex="-1" aria-hidden="true">
        <div class="pcp-al__img-wrap">
          ...
          <span class="pcp-al__type-badge">${oe[e.auctionType]||"🏷"} ${ie[e.auctionType]||e.auctionType}</span>
          ${fe}
        </div>
      </a>
      <div class="pcp-al__body">
        <p class="pcp-al__location">📍 ${a(((M=e.location)==null?void 0:M.city)||e.propertyCity||"")}</p>
        <h3 class="pcp-al__title"><a href="${C}">${a(e.title)}</a></h3>
        ${me}
        ...
        <a href="${C}" class="pcp-btn ${ye}">${ge}</a>
      </div>
    </article>`}function ue(e,t){e.querySelectorAll(".pcp-al__countdown[data-ends-at]").forEach(n=>{const a=new Date(n.dataset.endsAt).getTime();function r(){const o=a-Date.now(),i=(c,s)=>{const f=n.querySelector(`[data-part="${c}"]`);f&&(f.textContent=String(Math.max(0,s)).padStart(2,"0"))};if(o<=0){["d","h","m","s"].forEach(c=>i(c,0)),n.classList.add("pcp-al__countdown--expired");return}i("d",Math.floor(o/864e5)),i("h",Math.floor(o%864e5/36e5)),i("m",Math.floor(o%36e5/6e4)),i("s",Math.floor(o%6e4/1e3)),n.classList.toggle("pcp-al__countdown--urgent",o<3e5)}r(),t.timers.push(setInterval(r,1e3))})}function H(e,t,n){const a=e.querySelector(".pcp-al__pagination");if(t.totalPages<=1){a.innerHTML="";return}a.innerHTML=`
  ...
  `,a.querySelectorAll("button[data-dir]").forEach(r=>{r.addEventListener("click",()=>{t.page+=parseInt(r.dataset.dir,10),S(e,t,n)})})}return $("auction-listing",se),z}();
//# sourceMappingURL=pcp.min.js.map
